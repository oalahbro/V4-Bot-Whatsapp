...
const {
  ...
  createSerial,
  checkRegisteredUser,
} = require("../database/user/register.js");
...
module.exports = elaina = async (elaina, mek) => {
  try {
    ...
    const conts = mek.key.fromMe
      ? elaina.user.jid
      : elaina.contacts[sender] || { notify: jid.replace(/@.+/, "") };
    ...
    const mentions = (teks, memberr, id) => {
      id == null || id == undefined || id == false
        ? elaina.sendMessage(from, teks.trim(), extendedText, {
    ...
    const checkLimit = (sender) => {
      ...
      for (let lmt of _limit) {
        if (lmt.id === sender) {
          ...
          if (limitCounts <= 0)
            return elaina.sendMessage(from, `Limit kamu sudah habis`, text, {
              quoted: mek,
            });
          elaina.sendMessage(
            from,
    ...
    const isLimit = (sender) => {
      ...
      for (let i of _limit) {
        if (i.id === sender) {
          ...
          if (limits >= limitawal) {
            position = true;
            elaina.sendMessage(from, lang.limitend(pushname), text, {
              quoted: mek,
            });
            return true;
          } else {
    ...
    const sendMediaURL = async (to, url, text = "", mids = []) => {
      ...
      download(url, filename, async function () {
        ...
        elaina.sendMessage(to, media, type, {
          quoted: mek,
          mimetype: mime,
          caption: text,
    ...
    const sendFileFromUrl = async (from, url, caption, mek, men) => {
      ...
      mime = res.headers["content-type"];
      if (mime.split("/")[1] === "gif") {
        return elaina.sendMessage(from, await getBuffer(url), video, {
          caption: caption,
          gifPlayback: true,
          mentions: men ? men : [],
          mimetype: "video/mp4",
      ...
      if (mime === "application/pdf") {
        return elaina.sendMessage(from, await getBuffer(url), document, {
      ...
      if (mime.split("/")[0] === "image") {
        return elaina.sendMessage(from, await getBuffer(url), image, {
          caption: caption,
          mentions: men ? men : [],
          quoted: mek,
        });
      ...
      if (mime.split("/")[0] === "audio") {
        return elaina.sendMessage(from, await getBuffer(url), audio, {
          caption: caption,
          mentions: men ? men : [],
          mimetype: "audio/mpeg",
          quoted: mek,
    ...
    const sendButLocation = async (
      ...
      them = gam1;
      mediaxxaa = await elaina.prepareMessage(id, them, MessageType.location, {
        thumbnail: them,
      });
    ...
    const sendButImage = async (
      ...
      them = vid1;
      mediaxxaa = await elaina.prepareMessage(id, them, MessageType.image, {
    ...
    if (manti.includes("://chat.whatsapp.com/")) {
      ...
      elaina.groupRemove(from, [kic]).catch((e) => {
    ...
    if (
      siapakahaku.hasOwnProperty(sender.split("@")[0]) &&
      !isCmd &&
      !mek.key.fromMe
    ) {
      ...
      if (budy.toLowerCase() == jawaban) {
          ...
          [
            {
              ...
              buttonText: { displayText: "Main Lagi" },
              type: "RESPONSE",
            },
          ],
          { quoted: mek }
        ...
        fs.writeFileSync(
          "./database/siapakahaku.json",
          JSON.stringify(siapakahaku)
        );
      } else {
    ...
    switch (command) {
      ...
      case "owner":
        {
          ...
          for (let i of ownerContact.map((v) => v + "@s.whatsapp.net")) {
            const vname =
              elaina.contacts[i] != undefined
                ? elaina.contacts[i].vname || elaina.contacts[i].notify
                : undefined;
            ini_list.push({
              displayName: `${ownername}`,
              vcard: `BEGIN:VCARD\nVERSION:3.0\nN:Sy;Bot;;;\nFN:${vname}\nitem1.TEL;waid=${
          ...
          elaina.sendMessage(
            from,
            `Nih Kak Contact Owner Ku, Cuma Sv Nomor Cewe Ya `,
            text,
            { quoted: hehe }
          );
        }
      ...
      case "quotesimage":
        get_result = await getBuffer(
          `https://api.lolhuman.xyz/api/random/quotesimage?apikey=${lolkey}`,
          { method: "get" }
        );
      ...
      case "s":
        if (
          ...
          await ffmpeg(`./${media}`)
            ...
            .on("end", function () {
              console.log("Finish");
              elaina.sendMessage(from, fs.readFileSync(ran), sticker, {
                quoted: mek,
              });
              fs.unlinkSync(media);
              fs.unlinkSync(ran);
          ...
          .then(async (result) => {
            for (let i of result.medias) {
              if (i.url.includes("mp4")) {
                ...
                elaina.sendMessage(from, link, video, {
                  thumbnail: Buffer.alloc(0),
                  quoted: mek,
                ...
                let link = await getBuffer(i.url);
                elaina.sendMessage(from, link, image, {
                  thumbnail: Buffer.alloc(0),
                  quoted: mek,
                  caption: `Instagram  ${i.type}`,
                });
      ...
      case "tiktok":
        sendButLocation(
          ...
          [
            {
              buttonId: `.tiktokwm ${q}`,
              buttonText: { displayText: "WM" },
              type: 1,
            },
            {
              buttonId: `.tiktoknowm ${q}`,
              buttonText: { displayText: "NOWM" },
              type: 1,
            },
            {
              buttonId: `.tiktokmusic ${q}`,
      ...
      case "randommeme":
        get_result = await getBuffer(
          `https://api.lolhuman.xyz/api/random/meme?apikey=${lolkey}`,
          { method: "get" }
        );
        await elaina.sendMessage(from, get_result, image, { quoted: mek });
        break;
      case "memeindo":
        get_result = await getBuffer(
          `https://api.lolhuman.xyz/api/meme/memeindo?apikey=${lolkey}`,
          { method: "get" }
        );
        await elaina.sendMessage(from, get_result, image, { quoted: mek });
        break;
      case "darkjoke":
        get_result = await getBuffer(
          `https://api.lolhuman.xyz/api/meme/darkjoke?apikey=${lolkey}`,
          { method: "get" }
        );
        await elaina.sendMessage(from, get_result, image, {
          quoted: mek,
          caption: "Random Meme, Jangan Baper!!",
        });
        break;
      case "siapakahaku":
      case "siapaaku":
        {
          if (
            isGame(sender, isPremium, isCreator, isOwner, gcounttuser, glimit)
          )
            return sendButMessage(from, lang.limitg(prefix), ` ${ownername}`, [
              {
                buttonId: "limit",
                buttonText: { displayText: `Check Limit` },
                type: 1,
              },
            ]);
      ...
      case "tiktoknowm":
        ...
        zee
          ...
          .then((result) => {
            ...
            axios
              .get(`https://tinyurl.com/api-create.php?url=${nowm}`)
              .then(async (a) => {
                me = `*Link* : ${a.data}`;
                noweem = await getBuffer(nowm);
                elaina.sendMessage(from, noweem, MessageType.video, {
                  mimetype: "video/mp4",
                  quoted: mek,
                });
              });
      ...
      case "wallpaperanime":
        buffer = await getBuffer(
          `http://api.lolhuman.xyz/api/random2/wallpaper?apikey=${lolkey}`
        );
          ...
          .then((result) => {
            ...
            axios
              ...
              .then(async (a) => {
                ...
                elaina.sendMessage(from, weem, MessageType.video, {
                  mimetype: "video/mp4",
                  quoted: mek,
                });
              });
      ...
      case "quotesislami":
        anu = await fetchJson(
          `https://api.lolhuman.xyz/api/quotes/islami?apikey=${lolkey}`,
          { method: "get" }
        );
          ...
          .then((result) => {
            ...
            axios
              ...
              .then(async (a) => {
                ...
                elaina.sendMessage(from, audnha, MessageType.document, {
                  mimetype: "audio/mp4",
      ...
      case "jadwaltv":
        if (args.length == 0)
          return reply(
            `Nama Channel Nya Mana Tod\nContoh: ${prefix + command} SCTV`
          );
        channel = args[0];
        get_result = await fetchJson(
          `https://api.lolhuman.xyz/api/jadwaltv/${channel}?apikey=${lolkey}`
        );
      ...
      case "quotesdilan":
        quotedilan = await fetchJson(
          `https://api.lolhuman.xyz/api/quotes/dilan?apikey=${lolkey}`
        );
        reply(quotedilan.result);
        break;
      case "faktaunik":
        faktaunik = await fetchJson(
          `https://api.lolhuman.xyz/api/random/faktaunik?apikey=${lolkey}`
        );
      ...
      case "wikipedia":
        if (args.length == 0)
          return reply(
            `Nama Yg Mau Di Cari Mana Kak\nContoh: ${prefix + command} Tahu`
          );

        query = args.join(" ");

        get_result = await fetchJson(
          `https://api.lolhuman.xyz/api/wiki?apikey=${lolkey}`
        );
      ...
      case "translate":
        if (args.length == 0)
          return reply(
            `Teks Yg Mau Di Translate Mana Kak\nContoh: ${
              prefix + command
            } en Good Morning`
          );
        ...
        get_result = await fetchJson(
          `https://api.lolhuman.xyz/api/translate/auto/id?apikey=${lolkey}`
        );
        get_result = get_result.result;
      ...
      case "katabijak":
        anu = await fetchJson(
          `https://api.lolhuman.xyz/api/random/katabijak?apikey=${lolkey}`
        );
        reply(anu.result);
        break;
      case "pinterest":
      ...
      case "alquranaudio":
        if (args.length == 0)
          return reply(
            `Nomor Surah Yg Mau Dicari Mana\nContoh : ${
              prefix + command
            } 18 or ${prefix + command} 18/10`
          );
        ...
        await elaina.sendMessage(from, ini_buffer, audio, {
          quoted: mek,
      ...
      case "kisahnabi":
        if (args.length == 0)
          return reply(
            `Nama Nabi Yg Mau Dicari Mana\nContoh : ${
              prefix + command
            } Muhammad`
          );
        query = args.join(" ");
        get_result = await fetchJson(
          `https://api.lolhuman.xyz/api/kisahnabi/muhammad?apikey=${lolkey}`
        );
        get_result = get_result.result;
      ...
      case "jadwalsholat":
        ...
        daerah = args.join(" ");
        get_result = await fetchJson(
          `https://api.lolhuman.xyz/api/sholat/yogyakarta?apikey=${lolkey}`
        );
        get_result = get_result.result;
...
 Url : ${aramas.videos[0].url}`;
      ...
      case "verif":
        ...
        await sendButImage(
          from,
          lang.daftar(sender, pushname, time, serialUser, _registered),
          ` ${botname}`,
          await getBuffer(ppregis),
      ...
      case "memegen":
        {
          ...
          try {
            ...
            elaina.sendMessage(from, resu, image, {
      ...
      case "bot":
        ...
        elaina.sendMessage(from, sound, MessageType.audio, {
          quoted: mek,
          mimetype: "audio/mp4",
          ptt: true,
        });
      ...
      case "infogempa":
        ...
        await elaina.sendMessage(from, get_buffer, image, {
          quoted: mek,
          caption: ini_txt,
        });
        break;
      ...
      case "mangasearch":
        ...
        await elaina.sendMessage(from, thumbnail, image, {
          quoted: mek,
          caption: ini_txt,
        });
        break;
      case "joingrup":
        if (!mek.key.fromMe && !isOwner && !isCreator)
          return reply(lang.onlyOwner());
        try {
          if (!isUrl(args[0]) && !args[0].includes("whatsapp.com"))
            return reply(lang.erorLink());
          hen = args[0];
      ...
      case "broadcast":
        ...
        if ((isMedia && !mek.message.videoMessage) || isQuotedImage) {
          ...
          for (let _ of bcnya) {
            elaina.sendMessage(_.jid, bcnya3, image, {
              caption: `*----  BROADCAST ----*\n\n${q}`,
            });
          }
      ...
      case "nightcore":
        {
          ...
          exec(
            ...
            (err, stderr, stdout) => {
              ...
              elaina.sendMessage(from, hah, audio, {
                mimetype: "audio/mp4",
                ptt: true,
                quoted: mek,
      ...
      case "ppcp":
        anu = await fetchJson(
          `https://ziyy.herokuapp.com/api/ppcouple?apikey=xZiyy`
        );
        cowo = await getBuffer(anu.result.cowo_image);
        elaina.sendMessage(from, cowo, image, {
          caption: "Ini Untuk   *",
        });
          ...
          try {
            ...
            exec(
              ...
              (err, stderr, stdout) => {
                ...
                elaina.sendMessage(from, uhh, audio, {
                  mimetype: "audio/mp4",
                  ptt: true,
                  quoted: mek,
                });
      ...
      case "tebaklirik":
        anu = await fetchJson(
          `https://api.lolhuman.xyz/api/tebak/lirik?apikey=${lolkey}`,
          { method: "get" }
        );
        get = `*${anu.result.question}*`;
        setTimeout(() => {
          elaina.sendMessage(from, "Jawaban: " + anu.result.answer, text, {
            quoted: mek,
          });
        }, 60000); // 1000 = 1s,
        setTimeout(() => {
      ...
      case "suit":
        if (!q)
          return reply(
            `Kirim perintah ${prefix}suit [pilihan]\nContoh: ${prefix}suit gunting`
          );
      ...
      case "tebakkalimat":
        anu = await fetchJson(
          `https://api.lolhuman.xyz/api/tebak/kata?apikey=${lolkey}`,
          { method: "get" }
        );
        get = `*${anu.result.pertanyaan}*`;
        setTimeout(() => {
          elaina.sendMessage(from, "Jawaban: " + anu.result.jawaban, text, {
            quoted: mek,
          });
        }, 60000); // 1000 = 1s,
        setTimeout(() => {
      ...
      case "toimg":
        {
          ...
          exec(`ffmpeg -i ${mediaa} ${ran}`, (err) => {
            ...
            elaina.sendMessage(from, buffer, image, {
              quoted: mek,
              thumbnail: Buffer.alloc(0),
              caption: "Done",
            });
      ...
      case "facebookdl":
        {
          ...
          try {
            zee.Facebook(`${q}`).then(async (data) => {
              ...
              elaina.sendMessage(from, ppfb, video, {
                mimetype: "video/mp4",
                quoted: mek,
                caption: txt,
              });
      ...
      case "soundcloud":
        ...
        zee.SoundCloud(`${q}`).then(async (data) => {
          ...
          elaina.sendMessage(from, await getBuffer(data.medias[0].url), audio, {
            quoted: mek,
            mimetype: "audio/mp4",
          });
        });
      ...
      case "tstiker":
        {
          ...
          for (
            ...
            elaina.sendMessage(from, await getBuffer(telestc[i].url), sticker, {
              mimetype: "image/webp",
              quoted: mek,
            });
          }
      ...
      case "tebakgambar":
        ...
        hx.tebakgambar().then(async (data) => {
          ...
          console.log(jawaban);
          elaina.sendMessage(from, tebakya, image, {
            quoted: mek,
            caption: "\n\nTimeout : 120.00 seconds",
          });
          await sleep(120000);
