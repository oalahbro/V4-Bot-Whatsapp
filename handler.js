    ...
    for (let name in global.plugins) {
      ...
      if ((usedPrefix = (match[0] || "")[0])) {
        ...
        if (xp > 200)
          this.sendButton(
            m.chat,
            `[❗] *Sepertinya Anda Bermain Curang, Menggunakan Calculator*`,
            author,
            null,
            [
              ["Buy Limit", "/buy limit"],
...
export async function participantsUpdate({ id, participants, action }) {
  if (opts["self"]) return; // if (id in conn.chats) return // First login will spam
